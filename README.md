...
sequenceDiagram
    ...
    participant AL as ArrayList (Compartilhado)
    participant T2 as Thread 2
    
    Note over T1, T2: Ambas tentam adicionar um elemento simultaneamente
    T1->>AL: Lê índice atual para inserção (Ex: pos 5)
    T2->>AL: Lê índice atual para inserção (Ex: pos 5)
    T1->>AL: Escreve Objeto A na posição 5
    T2->>AL: Escreve Objeto B na posição 5 (SOBRESCRITA!)
    Note over AL: Objeto A foi perdido! O tamanho final será menor que o esperado.
```
